...
exports.create = async (req, res) => {
  const m = await Mechanic.create({ ...req.body, userId: req.user.id });
...
exports.addReview = async (req, res) => {
  ...
  const alreadyReviewed = m.reviews.find(
  ...
  m.reviews.push({ userId: req.user.id, rating: Number(rating), comment });
